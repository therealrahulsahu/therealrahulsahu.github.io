...
public class Solution1976  {
    public int countPaths(int n, int[][] roads) {
        ...
        Map<Integer, Integer> inQueue = new HashMap<>();
        ...
        Integer reached = 0;
        Integer firstReached = null;

        while(!pq.isEmpty()){
            Pair<Integer, Integer> top = pq.poll();
            Integer from = top.getKey();
            Integer cWgt = top.getValue();

            Integer count=1;
            Pair<Integer, Integer> next = pq.peek();
            while(next!=null && from.equals(next.getKey()) && cWgt.equals(next.getValue())){
                count++;
                pq.poll();
                next = pq.peek();
            }
//            for(Pair<Integer, Integer> next = pq.peek();next!=null && from.equals(next.getKey()) && cWgt.equals(next.getValue());next = pq.poll()){
//                count++;
//            }
            ...
            if(firstReached!=null && cWgt>=firstReached) continue;

            // if(visited.contains(from)) continue;
            visited.add(from);

            for(Pair<Integer, Integer> neigh: graph.get(from)){
                Integer to = neigh.getKey();
                Integer wgt = neigh.getValue();
                Integer times = count;

                if(distance.get(from)+wgt<=distance.get(to) && !visited.contains(to)){
                    distance.set(to, distance.get(from)+wgt);
                    while(times-->0) {
                        pq.offer(new Pair<>(to, distance.get(to)));
                    };
                }
            }
        }
...
//        int n=34;
        ...
        int n=7;
        System.out.println(new Solution1976().countPaths(n, roads));
    }
}
